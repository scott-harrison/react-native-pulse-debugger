...
const NetworkScreen: React.FC = () => {
    ...
    useEffect(() => {
        const handleMouseMove = (e: MouseEvent) => {
            ...
            const newWidth = Math.max(200, Math.min(800, dragStartWidth + deltaX));
    ...
    return (
        <div className="flex flex-1 h-full overflow-y-auto bg-gray-900/80">
            <div
                ...
                style={{
                    width: selectedRequest ? leftPanelWidth : '100%',
                    flexShrink: selectedRequest ? 0 : undefined,
                }}
            >
            ...
            {selectedRequest && (
                <div
                    className={cn(
                        'border-l cursor-col-resize hover:bg-purple-500/50 active:bg-purple-500/50 transition-all duration-200',
                        isDragging ? 'border-purple-500/50 bg-blue-500/50' : 'border-zinc-800'
                    )}
            ...
            {selectedRequest && (
                <div className="border-l border-zinc-800 flex flex-col flex-1 overflow-y-auto">
                                        ...
                                        Object.keys(selectedRequest.payload.headers).length > 0 && (
                                            <div>
                                                ...
                                                <div className="bg-zinc-900 p-2 rounded-md mt-1">
                                                    <JSONViewer
                                                        data={selectedRequest.payload.headers}
                                                    />
                                                </div>
                                            </div>
                                        )}
                                        ...
                                        <div>
                                            ...
                                            <div className="bg-zinc-900 p-2 rounded-md mt-1">
                                                {(() => {
                                                    try {
                                                        const data =
                                                            typeof selectedRequest.payload.body ===
                                                            'string'
                                                                ? JSON.parse(
                                                                      selectedRequest.payload.body
                                                                  )
                            ...
                            {selectedRequest.payload.response && (
                                <div>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        {(() => {
                                            const headers =
                                                selectedRequest?.payload?.response?.headers;
                                            if (
                                                !headers ||
