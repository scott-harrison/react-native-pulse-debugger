...
import { IEvent, INetworkPayload } from '@react-native-pulse-debugger/types';
...
function generateCurlCommand(requestPayload: INetworkPayload): string {
...
const NetworkScreen: React.FC = () => {
	...
	const handleCopyCurl = async (requestPayload: INetworkPayload) => {
												...
												{(() => {
													try {
														...
														const bodyString = String(
															typeof selectedRequest.payload.body === 'string'
																? selectedRequest.payload.body
																: JSON.stringify(selectedRequest.payload.body, null, 2)
														);
