...
export function getPulseState(
  ...
  console.log('[getPulseState] Checking connection state:', {
    hasProvidedClient: !!providedClient,
    hasModuleClient: !!client,
  });

  const targetClient = providedClient || client;
  if (!targetClient) {
    console.log('[getPulseState] No client available');
    return 'disconnected';
  }

  const isConnected = targetClient.isConnected();
  console.log('[getPulseState] Connection check result:', isConnected);
