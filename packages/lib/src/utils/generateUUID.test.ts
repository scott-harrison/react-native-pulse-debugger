...
describe('generateUuid', () => {
  ...
  it('should generate a valid UUID v4 string', () => {
    ...
    // Note: The 6th byte (index 5) is set to version 4, and 8th byte (index 7) is set to variant 1
  ...
  it('should generate unique UUIDs on each call', () => {
    ...
    // UUIDs should be different even though version and variant bits are set
    expect(uuid1).not.toBe(uuid2);
    expect(mockRandomValues).toHaveBeenCalledTimes(2);

    // Both UUIDs should still be valid v4 UUIDs
