...
import type { ValidationResult, EventHandler, EventMessage } from './types';
...
export class EventManager {
   ...
   */
  public off(
    type: OutgoingEventType | IncomingEventType | string,
   ...
   */
  public emit<T>(type: OutgoingEventType | string, payload: T): void {
    const validation = this.validateMessage(type as OutgoingEventType, payload);
    if (!validation.isValid) {
  ...
  private validateMessage(
    type: OutgoingEventType | IncomingEventType,
    payload: unknown
  ): ValidationResult {
